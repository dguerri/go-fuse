...
// binary. macFUSE 5.2+ installs this; earlier versions and Linux/FreeBSD do not.
...
// newCachingLoader returns a loader that calls inner at most once and caches
// the result (success or failure). Each call to newCachingLoader creates an
// independent cache, so production code uses a single package-level instance
// (see prodLoader) while tests can construct their own.
func newCachingLoader(inner mfMountLoader) mfMountLoader {
	var (
		once sync.Once
		fn   mfMountFunc
		err  error
	...
	return func() (mfMountFunc, error) {
		once.Do(func() { fn, err = inner() })
		return fn, err
	}
}

// prodLoader is the production singleton used by mount_darwin.go's switch.
// dlopen happens at most once per process.
var prodLoader = newCachingLoader(realLoadMFMount)
